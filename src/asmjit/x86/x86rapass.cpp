...
static OperandSignature x86VecRegSignatureBySize(uint32_t size) noexcept {
  return OperandSignature(size >= 64 ? Zmm::kSignature :
                          size >= 32 ? Ymm::kSignature : Xmm::kSignature);
}
...
Error X86RAPass::emitMove(uint32_t workId, uint32_t dstPhysId, uint32_t srcPhysId) noexcept {
  ...
  if (_formatOptions.hasFlag(FormatFlags::kAnnotations)) {
