...
Error Assembler::_emit(InstId instId, const Operand_& o0, const Operand_& o1, const Operand_& o2, const Operand_* opExt) {
  ...
  if (ASMJIT_UNLIKELY(Support::test(options, kRequiresSpecialHandling))) {
    ...
    // Strict validation.
    if (hasValidationOption(ValidationOptions::kValidateAssembler)) {
  ...
  switch (instInfo->_encoding) {
    ...
    case InstDB::kEncodingBaseBranchRel: {
      ...
      if (isign4 == ENC_OPS1(Label) || isign4 == ENC_OPS1(Imm)) {
        ...
        if (Support::test(options, InstOptions::kARM_CondFlagMask)) {
          uint32_t cond = (uint32_t(options) >> Support::ConstCTZ<uint32_t(InstOptions::kARM_CondCodeMask)>::value) & 0xFu;
          if (cond != Cond::kAL) {
            opcode |= B(30);
            opcode.addImm(cond, 0);
            offsetFormat.resetToImmValue(OffsetType::kCommon, 4, 5, 19, 2);
            goto EmitOp_Rel;
          }
        }

        offsetFormat.resetToImmValue(OffsetType::kCommon, 4, 0, 26, 2);
        goto EmitOp_Rel;
      }
    ...
    case InstDB::kEncodingBaseBranchTst: {
      ...
      if (isign4 == ENC_OPS3(Reg, Imm, Label) || isign4 == ENC_OPS3(Reg, Imm, Imm)) {
        ...
        offsetFormat.resetToImmValue(OffsetType::kCommon, 4, 5, 14, 2);
